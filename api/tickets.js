export default async function handler(req, res) {
  ...
  if (!domain || !email || !token) return res.status(400).json({ error: 'domain, email e token são obrigatórios' });

  const base64 = Buffer.from(`${email}/token:${token}`).toString('base64');
  const perPage = Math.min(100, limit);

  const parts = ['type:ticket'];
  if (channel !== 'all') parts.push(`via:${channel}`);
  ...
  try {
    ...
    return res.status(200).json({ tickets: data.results || [], count: data.count || 0, next_page: data.next_page || null });
  } catch (err) {
